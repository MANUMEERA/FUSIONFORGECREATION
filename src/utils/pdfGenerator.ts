...
export function generateInvoicePDF(invoice: Invoice, customAgencyConfig?: any) {
  ...
  const isUrp = !rawBuyerGstin || rawBuyerGstin === '—' || rawBuyerGstin.toUpperCase() === 'URP';
  const buyerGstin = isUrp ? 'URP (Unregistered Person)' : rawBuyerGstin;
  const buyerState = invoice.buyerState || (invoice.buyerStateCode ? `${invoice.buyerStateCode}-State` : '24-Gujarat');
  const placeOfSupply = invoice.placeOfSupply || invoice.buyerStateCode || '24-Gujarat';

  // Shipping values
  const hasDistinctShipping = invoice.sameAsBilling === false && (invoice.shippingAddress || invoice.shippingCity);
  const shippingName = invoice.shippingName || invoice.shippingCompany || buyerCompany;
  const shippingAddress = invoice.shippingAddress || buyerAddress;
  const shippingCity = invoice.shippingCity || '';
  const shippingState = invoice.shippingState || (invoice.shippingStateCode ? `${invoice.shippingStateCode}-State` : '');
  const shippingPincode = invoice.shippingPincode || '';
  const shippingGstin = invoice.shippingGstin || buyerGstin;
    ...
    <html>
      <head>
        ...
        <style>
          ...
          .parties-grid {
            display: grid;
            grid-template-columns: ${hasDistinctShipping ? '1fr 1fr 1fr' : '1fr 1fr'};
            gap: 16px;
            margin-bottom: 24px;
          }
          .party-card {
      ...
      <body>
        ...
        <div class="parties-grid">
          ...
          <div class="party-card">
            <div class="party-header">Seller (Supplier):</div>
            ...
            ${cfg.msme_number ? `<div class="party-detail"><strong>MSME / Udyam:</strong> ${cfg.msme_number}</div>` : ''}
          </div>

          <!-- Billed To (Buyer) Details -->
          <div class="party-card">
            <div class="party-header">Billed To (Recipient):</div>
          ...
          <!-- Shipped To Details -->
          <div class="party-card">
            <div class="party-header">Shipped To (Delivery):</div>
            ...
            ${shippingGstin ? `<div class="party-detail"><strong>GSTIN:</strong> ${shippingGstin}</div>` : ''}
          </div>
          ` : ''}
        </div>
        ...
        <div class="sig-row">
          <div style="font-size: 11px; color: #64748b;">
            This is a computer-generated tax invoice and legally compliant under GST rules.<br/>
            Issued by <strong>${sellerName}</strong>
          </div>
          <div style="text-align: right; position: relative; min-width: 180px;">
            <div style="font-size: 12px; font-weight: 700; color: #0f172a;">For ${sellerName}</div>
            ${cfg.signature_url ? `
              <div style="margin-top: 8px; margin-bottom: 4px; display: flex; justify-content: flex-end;">
                <img src="${cfg.signature_url}" alt="Signature" style="max-height: 48px; max-width: 140px; object-fit: contain;" />
              </div>
            ` : `
              <div style="margin-top: 36px;"></div>
            `}
            ${cfg.stamp_url ? `
              <div style="position: absolute; right: 80px; bottom: 5px; opacity: 0.85; pointer-events: none;">
                <img src="${cfg.stamp_url}" alt="Stamp" style="max-height: 55px; max-width: 55px; object-fit: contain;" />
              </div>
            ` : ''}
            <div style="font-weight: 800; color: #0f172a; border-top: 1px solid #0f172a; display: inline-block; padding-top: 4px; font-size: 12px;">
