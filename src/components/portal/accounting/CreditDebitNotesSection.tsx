...
export const CreditDebitNotesSection: React.FC = () => {
  ...
  const [noteToDelete, setNoteToDelete] = useState<CreditDebitNote | null>(null);
  ...
  // Delete Note
  const handleDelete = (note: CreditDebitNote) => {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs">
          <div className="w-full max-w-md bg-white border border-red-200 rounded-3xl shadow-2xl p-6 relative text-[#1E1B2E]">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h2 className="text-lg font-bold text-[#1E1B2E]">Delete {noteToDelete.noteType === 'credit' ? 'Credit' : 'Debit'} Note</h2>
            ...
            <p className="text-xs text-[#5F5A72] mb-4 leading-relaxed">
              Are you sure you want to delete <strong className="text-[#1E1B2E]">{noteToDelete.noteNumber}</strong> issued to <strong className="text-[#1E1B2E]">{noteToDelete.clientCompany || noteToDelete.clientName}</strong> (₹{noteToDelete.totalAmount.toLocaleString('en-IN')})?
            </p>
            ...
            <div className="pt-3 border-t border-[#E8E0F0] flex items-center justify-end space-x-2">
              ...
              <button
                type="button"
                onClick={async () => {
                  await deleteCreditDebitNote(noteToDelete.id);
                  success('Note Deleted', `Note ${noteToDelete.noteNumber} removed.`);
