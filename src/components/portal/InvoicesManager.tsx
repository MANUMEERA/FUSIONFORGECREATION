...
export const InvoicesManager: React.FC = () => {
  ...
  const [deleteConfirmInvoice, setDeleteConfirmInvoice] = useState<{ invoice: Invoice; isPermanent: boolean } | null>(null);
  ...
  return (
    <div className="space-y-6">
                    ...
                    <tr key={inv.id} className="hover:bg-[#FAF8FF] transition-colors">
                      ...
                      <td className="py-4 px-4 text-right">
                        <div className="flex items-center justify-end space-x-1.5">
                          ...
                          {isSuperAdmin && !inv.isDeleted && (
                            <button
                              onClick={() => setDeleteConfirmInvoice({ invoice: inv, isPermanent: false })}
                            ...
                            <>
                              <button
                                onClick={() => {
                                  ...
                                  success('Invoice Restored', `Invoice ${inv.invoiceNumber} restored from Trash.`);
                                }}
                              ...
                              {isSuperAdmin && (
                                <button
                                  onClick={() => setDeleteConfirmInvoice({ invoice: inv, isPermanent: true })}
              ...
              <div>
                ...
                <div className="mt-2 text-xs text-[#5F5A72] space-y-0.5">
                  ...
                  <div className="flex flex-wrap gap-x-3 text-[11px] text-[#817B91]">
                    ...
                    {agencyConfig.pan && <span><strong className="text-[#1E1B2E]">PAN:</strong> {agencyConfig.pan}</span>}
                    {agencyConfig.email && <span><strong className="text-[#1E1B2E]">Email:</strong> {agencyConfig.email}</span>}
                    {agencyConfig.phone && <span><strong className="text-[#1E1B2E]">Contact:</strong> {agencyConfig.phone}</span>}
                  </div>
      ...
      {/* Invoice Delete / Move to Trash Confirmation Modal */}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs">
          <div className="w-full max-w-md bg-white border border-red-200 rounded-3xl shadow-2xl p-6 relative text-[#1E1B2E]">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h2 className="text-lg font-bold text-[#1E1B2E]">
                  {deleteConfirmInvoice.isPermanent ? 'Permanently Delete Invoice' : 'Move Invoice to Trash'}
                </h2>
                <p className="text-xs text-red-600 font-semibold">
                  {deleteConfirmInvoice.isPermanent ? 'Confirm permanent removal' : 'Confirm soft deletion'}
                </p>
              </div>
            </div>

            <p className="text-xs text-[#5F5A72] mb-4 leading-relaxed">
              {deleteConfirmInvoice.isPermanent ? (
                <>
                  Are you sure you want to permanently erase invoice <strong className="text-[#1E1B2E]">{deleteConfirmInvoice.invoice.invoiceNumber}</strong> ({deleteConfirmInvoice.invoice.clientCompany || deleteConfirmInvoice.invoice.clientName})? This action cannot be undone.
                </>
              ) : (
                <>
                  Are you sure you want to move invoice <strong className="text-[#1E1B2E]">{deleteConfirmInvoice.invoice.invoiceNumber}</strong> to Trash? You can restore it later if needed.
                </>
              )}
            </p>
            ...
            <div className="pt-3 border-t border-[#E8E0F0] flex items-center justify-end space-x-2">
              ...
              <button
                type="button"
                onClick={() => {
                  if (deleteConfirmInvoice.isPermanent) {
                    deleteInvoice(deleteConfirmInvoice.invoice.id);
                    success('Invoice Deleted', `Invoice ${deleteConfirmInvoice.invoice.invoiceNumber} permanently removed.`);
                  } else {
                    softDeleteInvoice(deleteConfirmInvoice.invoice.id);
                    success('Moved to Trash', `Invoice ${deleteConfirmInvoice.invoice.invoiceNumber} moved to Trash.`);
                  }
              ...
              >
                <Trash2 className="w-3.5 h-3.5" />
                <span>{deleteConfirmInvoice.isPermanent ? 'Delete Forever' : 'Move to Trash'}</span>
              </button>
            </div>
          </div>
        </div>
